...
const STORAGE_KEY = "samudrasetu:user"

// Persist and read the current user from localStorage to keep role across reloads
export const getCurrentUser = (): User | null => {
  // try reading from localStorage first (browser only)
  if (typeof window !== "undefined") {
    try {
      ...
      // ignore parse errors, fall back to in-memory
    }
  }
  return currentUser
}
...
export const canAccessDashboard = (user: User | null): boolean => {
  return user?.role === "admin" || (user?.role === "official" && user.isVerified)
}
